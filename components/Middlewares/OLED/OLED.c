...
void OLED_Update_RTOS(void) {
    ...
    if (!I2C_Bus_IsTxReady()) return;
    
    OLED_SetCursor(page, 0);
    
    // 构造要发送的数据：控制字节 + 128字节显存
    uint8_t buf[1 + 128];
    buf[0] = OLED_WRITE_DATA_ADD;
    memcpy(buf + 1, OLED_DisplayBuf[page], 128);
    
    // 异步发送（不等待完成）
    I2C_Bus_TransmitAsync(OLED_ADDRESS << 1, buf, 1 + 128);
